...
const services = [
  { name: "Full Stack Development (MERN)", src: "magic-wand" },
  { name: "Frontend Development", src: "paint-bucket" },
  { name: "Mobile Development(React Native)", src: "web" },
  { name: "Backend & REST API Development", src: "world" },
  { name: "Responsive Web Design", src: "planet" },
  { name: "JWT Authentication & Security", src: "cube" },
  { name: "Payment Gateway Integration", src: "cube" },
]
            ...
            {tech.map(({ name, src }) => (
              <MotionLi key={name} variants={element}>
                <div className="group relative">
                  <input placeholder={name} type="checkbox" className="peer hidden" id={name.replace(/\s+/g, '-').toLowerCase()} />

                  <label
                    htmlFor={name.replace(/\s+/g, '-').toLowerCase()}
